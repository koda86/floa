singlecurve_coverage <- function (data, n.boot) {
  ...
  for (curve.idx in n.curves) {
    ...
    cover.cross.rcb.v1[curve.idx] <- get_coverage(data.subset, floa.v1)
    ...
    cover.cross.rcb.v2[curve.idx] <- get_coverage(data.subset, floa.v2)

    cover.cross.rcb.v3[curve.idx] <- get_coverage(data.subset, floa.v3)

    cover.cross.roislien[curve.idx] <- get_coverage(data.subset, floa.roislien)
