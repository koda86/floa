...
Das punktweise Vorgehen hat noch einen anderen möglichen Vorteil gegenüber funktionalen Methoden: Sie können auf alle Arten kontinuierlicher Daten angewendet werden. Im Gegensatz zu den hier untersuchten glatten Kurven lassen sich mathematische Funktionen auf Signale mit vielen sprunghaften Veränderungen im Signalverlauf wie z. B. EMG-Signale nicht fitten. Die Untersuchung dieser Art von Kurven war allerdings nicht Gegenstand des vorliegenden Papers. Die hier vorgestellten Datensätze decken dennoch ein breites Spektrum biomechanischer Signale ab.

#### Auswahl geeigneter Fehlerbänder
Abgesehen von Unterschieden in der Berechnung und den coverage-Eigenschaften ist allen drei untersuchten Methoden gemein, dass Prädiktionsintervalle konstruiert werden. Um eine möglichst hohe coverage zu erzielen fallen diese Intervalle daher gezielt breit aus. Das ist im Sinne von Bland & Altman erwünscht um einen möglichst hohen Anteil von Einzelbeobachtungen in den Intervallen zu enthalten und so eine gewisse Sicherheit zu haben, dass auch die nächste Einzelmessung im Intervall liegt. Im biomechanischen Kontext ist das z. B. dann relevant, wenn neue Messwerte gegen normative Daten (z. B. aus einer Referenzdatenbank) verglichen werden. Wenn hier die Abweichung ein bestimmtes Maß überschreitet, d. h. der Proband nicht zur Referenzpopulation gehört, werden bestimmte pathologische Muster angenommen und eine geeignete Therapie vorgeschlagen (Olshen et al., 1989). Entsprechend ist in diesem Kontext von Bedeutung, dass die Bänder nicht zu eng ausfallen, weil die Konsequenzen falsch positiver Schlüsse schwerwiegend sein können.

Für viele andere Fragestellungen sind die breiten Prädiktionsintervalle allerdings nicht geeignet weil breitere Intervalle ein höheres Risiko für falsch negative Schlüsse bedeuten. Das ist z. B. der Fall wenn die Messungen im Kontext eines Vergleichs verschiedener Probandenkollektive erhoben werden (z. B. in Interventionsstudien oder case-control Studien). Gut demonstrieren lässt sich das am Beispiel der Differenzen in Fig. 2 D (Differenzen zwischen einem optischen motion capture System und einer IMU). Eine mittels IMU gemessene Differenz zwischen den sagittalen Hüftwinkeln z. B. einer Kontroll- und Interventionsgruppe kann hier nur dann als tatsächlicher Effekt angenommen werden, wenn sie mehr als die (je nach Intervall ungefähr) $\pm$ 10° des zufälligen Messfehlers beträgt. Derartig große Effekte sind in biomechanischen Untersuchungen allerdings nur selten zu erwarten.
<!-- Dies gilt umso mehr vor dem Hintergrund häufig eher geringer Effektstärken in der Biomechanik (Quelle ... Knudson? Metaanalysen!?).  -->
<!-- Typische Effektgrößen liegen hier eher bei ... (Quelle). -->
Die hier beobachtete Streuung der Differenzen ist dabei längst kein Szenario das weit von der Praxis entfernt ist. Gerade bei markerbasierten motion capture Systemen, die häufig als ground truth eingesetzt werden, ist die Reliabilität aufgrund von u. a. Weichteilbewegungen, Markerplatzierungsartefakten etc. fragwürdig, so dass hier leicht sehr breite (Prädiktions)Intervalle entstehen können.
<!-- Werte/Studie zur Reliabilität von MoCap Systemen? -->

Eine Alternative zu den breiten Prädiktionsintervallen sind Intervalle auf Grundlage der within-subject Standardabweichung (aus einer einfaktoriellen Varianzanalyse) (Bland & Altman, 1999). Wiederum am Beispiel Hüftgelenksdifferenzen (Fig. 2, D) lässt sich erkennen, dass die Streuung innerhalb einzelner Probanden erheblich geringer ausfällt (erkennbar an den probandenweisen Kurvenclustern). Diese Eigenschaft kann ausgenutzt werden um engere Intervalle auf Grundlage der intraindividuellen Streuung zu konstruieren wenn primär die Reproduzierbarkeit der neuen Methode von Bedeutung ist. Das ist z. B. der Fall wenn Untersuchungen mit einem reinen within-subjects Design durchgeführt werden (z. B. bei prä-post-Vergleichen in Interventionsstudien). Neben der Tatsache dass diese Intervalle bereits für sich gesehen informativ sind, stellen sie auch eine Art Baseline zur Bewertung der between-method variability dar.

In den Fällen wo Messsysteme zunächst validiert werden ohne dass unmittelbar klar ist für welche Anwendung das Messsystem später eingesetzt werden soll empfehlen wir verschiedene Bänder für within und between-subject Designs zu konstruieren.

Falls in der Praxis als zu aufwändig bewertet wird, scheinen die untersuchten POINT-Bänder (mit messwiederholten Werten!) einen brauchbaren Kompromiss zwischen zu breiten und zu engen Intervallen darzustellen. Zudem sollten berechnete Fehlerintervalle stets grafisch anhand der Messdaten (Originalkurven und/oder Differenzkurven) evaluiert werden.

In jedem Fall sollten aber aus den in der Einleitung genannten Gründen auf kontinuierliche Methoden gesetzt werden. Diese müssen nicht zwingend mithilfe funktionaler Verfahren berechnet werden, sondern punktweise konstruierte Bänder können - je nach Anwendungsfall - durchaus gute Ergebnisse liefern.

Wir spekulieren dass der seltene Einsatz kontinuierlicher Verfahren in direktem Zusammenhang mit der Komplexität der Verfahren steht, die eine hohe Hürde für viele Praktiker mit weniger ausgeprägten Statistik- und Programmierkenntnissen darstellt. Um die Verbreitung funktionaler Methoden zu katalysieren stellen wir daher die bei der Erstellung des Papers implementierten Methoden als R Code frei zur Verfügung (GPL).
