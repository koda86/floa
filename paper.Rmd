...
### TODO
- Punktweise LoA wasserdicht machen (Parker et al., 2020)
<!-- - Alternativtitel: Comparing different methods for constructing continuous prediction bands -->
...
* 1. FLoA~RCB_v2~ : One random stride per subject

* 2. FLoA~RCB_v3~ : Fetch a SINGLE random stride from all strides

* 3. FLoA~Roislien~ : Roislien approach (Get one random stride from each subject ONCE and boot-
       strap the resulting sample (of length (n=length(subjects))

* 4. FLoA~Point~ : Pointwise B & A Limits of Agreement (SD derived from linear mixed effects model)
...
colnames(covered.curves.percent) <- c("RCB_2", "RCB_3", "Roislien_4", "Point_5")
...
knitr::kable(cover.cross.mean.sd, caption = "Mean (top) and SD of covered points")
