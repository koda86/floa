...
Hier ein anderes Intro schreiben ... Ansatz: Neben der Validierung von Messgeräten spielt auch die Validierung von simulierten Modellergebnissen eine Rolle.
...
To compare different methods for constructing continuous LoA, smooth curve data were simulated across a range of common biomechanical signals and error characteristics.

The curves were modeled as Fourier series from additively superimposed sine functions. Each data set contains n.strides = 10 curves from n.subj = 11 fictitious test subjects. Each curve consists of t = 101 data points. Note that the curves do not necessarily correspond to real-world signals as we were more concerned with illustrating the underlying principle.

<!-- Aus Robinson et al. (2021) -->
<!-- Exemplar 1D eects from the biomechanics literature across a range of signal types were collated and reported (Bakke & Besier, 2020, Barrios & Willson, 2017, Bovi, Rabuetti, Mazzoleni, & Ferrarin, 2011, Phan et al., 2017, Gomes, Ackermann, Ferreira, Orselli, & Sacco, 2017, Robinson et al., 2014). -->

<!-- the code to reproduce all 1D eects is provided at https://github.com/m-a-robinson/sample-size -->
...
* (2) Same model as in (i), but with a nonlinear trend underlying one of the two signals. Similar errors can be observed e. g. when joint angles are calculated (numerically integrated) from gyroscope signals.
...
* (3) Heteroscedasticity across the signal. The example consists of knee momentcurves calculated using two different methods (direct and inverse kinematic approach) (Robinson et al., 2014). The curves were generated using the example code in Robinson et al. (2021).
