---
title: "Comparison of continuous prediction bands for the validation of smooth 1D biomechanical time series data"
...
Für den Vergleich der Methoden wurden vier (simulierte und reale) Datensätze verwendet. Jeder der Datensätze enthält jeweils $N = 220$ Kurven der Länge $T = 101$ Datenpunkte aus $M = 2$ Messsystemen $(m=1,2)$. Das entspricht in jedem Datensatz $D = 110$ Differenzkurven, die durch Subtraktion gebildet werden:

$\sum D_{ij} = \sum_{i=1}^{I} \sum_{j=1}^{J} Y_{ij} - X_{ij} = 110$

X und Y sind (I x J) Matrizen der beiden Messsysteme, mit $I = 11$ Probanden und $J = 10$ wiederholten Kurven pro Proband. Die Größe der Datensätze entspricht in etwa dem typischen Stichprobenumfang in Validierungsstudien.

<!-- Die $nN$ Kurven entsprechen $J = 10$ repeated measures von $I = 11$ Probanden und $m = 2$ Messsystemen: -->
<!-- $n = m * \sum_{i=1}^{11} \sum_{j=1}^{10} n_{ij} = 220$ -->

Alle simulierten Datensätze wurden als glatte Kurven, ähnlich Gelenkwinkelverläufen, mit dem gleichen Grundmodell generiert. Die Kurven korrespondieren nicht mit realen Signalen und wurden teilweise bewusst überspitzt konstruiert um den Einfluss bestimmter Differenzcharakteristika auf das Verhalten der Bänder möglichst deutlich darzustellen. Die Kurven wurden als trigonometrische Funktionen durch Addition zweier Sinusfunktionen konstruiert:

$$
X_{ij}(t) = (offset_{m}) + f_{1} + f_{2}, Y_{ij}(t) = (offset_{m}) + f_{3} + f_{4}
$$
mit
...
Um die untersuchten Methoden im gleichen Wahrscheinlichkeitsraum (probability space, function space?) zu analysieren, wurden Differenzkurven zwischen den beiden Systemen als Berechnungsgrundlage gebildet. Im Kontext von Messsystemvalidierungen bietet sich die Verwendung von Differenzkurven zudem an, weil mit diesen Kurven unmittelbar die (von Bland & Altman formulierte) Fragestellung beantwortet wird, inwiefern die verglichenen Methoden in der Praxis austauschbar eingesetzt werden können?

Auf Grundlage der Differenzkurven wurden Prädiktionsintervalle mit drei unterschiedlichen Methoden berechnet:

* 1. POINT : Pointwise continuous LoA according to Bland & Altman (1999, 2007)

Für alle $t = 101$ Punkte wurden jeweils über alle $d$ Kurven Bland & Altman LoA berechnet. Wenn diese Differenzen normalverteilt (homoskedastisch, unabhängig) sind, kann erwartet werden, dass 95% der Differenzen im Intervall mean difference $\pm$ 1.96 $*$ standard deviation of the differences liegen. Dieses einfache Modell kann erweitert werden wenn die Modellvoraussetzungen nicht ausreichend erfüllt sind (z. B. nichtnormalverteilte Differenzen (logarithmische Transformation), Abhängigkeit der Differenzen vom zugehörigen Mittelwert (Regressionsansatz), Abhängigkeit der Kurven (messwiederholte Daten)).

Wir verwenden in der vorliegenden Arbeit ein Modell für wiederholte Messungen, in dem sich der wahre Wert über jedes Differenzpaar ändert. Das Modell beschreibt die Differenz zwischen zwei Messsystemen als Summe der mittleren Differenz ($\overline{d}$) und zweier Varianzkomponenten (between (\sigma^2_{db}), within (\sigma^2_{dw})):
...
$$
Die beiden Varianzkomponenten können mithilfe einer einfaktoriellen Varianzanalyse bestimmt werden. \sigma^2_{dw} wird dabei als residual mean square ($MS_{w}$) direkt aus dem ANOVA table abgelesen. \sigma^2_{db} berechnet sich aus der Differenz aus between-subject mean squares und $MS_{w}$, geteilt durch einen Term, der die Anzahl der $i$ Probanden und $j$ Kurven berücksichtigt. Wenn für alle Probanden die gleiche Anzahl an Datenpunkten vorliegt (wie im vorliegenden Fall), reduziert sich dieser Term auf die Anzahl der Datenpunkte (Kurven) pro Proband ($j = 10$):

$$
\sigma^2_{db} = \frac{(MS_{b} - MS_{w})}{j}
$$

Für weiterführende Informationen zur Berechnung (z. B. bei ungleicher Anzahl Observationen pro Proband), siehe Bland & Altman (1999) sowie die zugehörige Implementierung als R Code im Anhang.

* 2. ROISLIEN : Functional limits of agreement (FLoA) according to Roislien et al. (2012)

Die Methode von Røislien et al. (2012) wurde als Erweiterung der Methode von Bland & Altman (1983) für kontinuierliche Differenzen ($d(t)$) entwickelt. Roislien et al. approximieren die Kurven im Datensatz daher zunächst mithilfe elementarer trigonometrischer Funktionen. Die Berechnung der Modellparameter (mean, standard deviation) erfolgt im Anschluss jedoch - genau wie in POINT - separat für jeden Punkt der Kurve. Daher macht es in der praktischen Umsetzung de facto keinen Unterschied ob die originalen (diskreten) oder funktionalen Kurven verwendet werden. Wir verzichten daher im Rahmen dieses Papers auf die Repräsentation der Kurven als Funktionen. Damit ergeben sich gegenüber POINT zwei Unterschiede:

(i) Da die Unabhängigkeit der Kurven vorausgesetzt wird, werden keine wiederholten Kurven verwendet, sondern je eine Kurve pro Proband ausgewählt. Es wird also nicht wie bei POINT mit allen Kurven gerechnet, sondern mit einer reduzierten Version des Datensatzes. 

(ii) Die (punktweise) Berechnung der LoA erfolgt für diese $j = i$ Kurven mithilfe des minimalen Bland & Altman Modells als $FLoA = d(t) \pm 1.96SD(t)$ berechnet, wobei $\overline{d}(t)$ die kontinuierliche mean difference bezeichnet. 

* 3. BOOT : Boostrapped functional prediction bands according Sutherland et al. (1988), Olshen et al. (1989) and Lenhoff et al. (1999)

Die Methode

Das Bootstrapping der Prädiktionsintervalle wird nicht mit diskreten Werten gemessener Kurven durchgeführt, sondern mit Funktionen der Kurven. Die Funktionen werden durch Kurvenanpassung mit Fourier-Reihen erzeugt. Kurvenanpassung ist notwendig, da diskrete Messwerte eines Kurvenverlaufs keine unabhängig voneinander auftretenden Ereignisse sind, die einer Normalverteilung unterliegen.

Im ersten Schritt des Verfahrens werden alle gemessenen Kurven (n) mithilfe von Fourier-Reihen als Funktionen modelliert:

$$
f(t) = \mu_{i} + \sum_{k=1}^K(\alpha_{i, k}cos(\frac{2\pi kt}{T}) + \beta_{i, k} sin(\frac{2\pi kt}{T}))
$$
Die Freiheitsgrade $K$ sind dabei so zu wählen, dass die Kurven ausreichend gut angepasst werden. In der vorliegenden Arbeit wurden die Kurven großzügig mit K = 50 approximiert. Die Koeffizienten der Kurven werden durch die Methode der kleinsten Quadrate angenähert. Für das weitere Vorgehen ist dabei wichtig, dass $\mu$ für alle Kurven 0 sein muss, da die Standardabweichungskurve der Kurvenschar sonst einen Offset aufweist. Durch die Kurvenanpassung wird für jede Kurve ein Vektor mit $2k+1$ Koeffizenten erzeugt:

$$
W_{i} = (\mu_{i}, \alpha_{i, 1}, \beta_{i, k}, ..., \beta_{i, K})
$$
Die Vektoren werden in einer Matrix M mit den Dimensionen n (Zeilen) und 2k+1 (Spalten) zusammengefasst. Durch Mittelwertbildung und Berechnung der Varianz über die Spalten können jetzt Vektoren der Mittelwert- (W) und der Varianzkoeffizienten (Σw) berechnet werden. Durch Multiplikation von W (Formel 18) und Σw (Formel 19) mit

$$
l(t) = (1, cos(\frac{2\pi 1t}{T}, sin(\frac{2\pi 1t}{T}, ..., cos(\frac{2\pi Kt}{T}, sin(\frac{2\pi Kt}{T},))
$$
lassen sich die Mittelwertkurve

$$
\hat{f}(t) = \overline{W}^T l (t)
$$
und der Verlauf der Standardabweichung 

$$
\hat{\sigma}_{f(t)} = \sqrt(l(T)^T \sum_{w} l(t))
$$
der Kurvenschar anpassen.

Anschließend werden aus der Matrix M die Koeffizienten von n Kurven im
Bootstrappingverfahren b mal gezogen (Ziehen mit Zurücklegen). Jede Bootstrap-Wiederholung erzeugt ein 'bootstrap sample' (Bootstrap-Ziehung) entsprechend der ursprünglichen Kurvenanzahl n. Aus den Koeffizienten der n Kurven jeder Bootstrap-Ziehung werden mit
...
$$
P \{\max_{t} (\frac{|\hat{f}_{n+1}(t) - \hat{f}(t)|}{\hat{\sigma}_{f(t)}} \le C_{p}\} = 1-\alpha
$$
...
Let fn + 1(t) denote a future draw from the population mean curve that is defined by Eq.
(0.1) and let Yn + 1(t) be an observed curve generated by Eqs. (0.2) and (0.3). Given a desired confidence level 100(1− a)%, we chose the constant Cp so that
...
Für weitere Details zur Methode siehe Lenhoff et al. (1999), Appendix A, sowie die zugehörige Implementierung als R Code im Anhang.
...
Für die punktweisen wurde 

### LoA uncertainty estimation

Um die Schwankung der Intervallgrenzen über wiederholte Berechnungen hinweg zu veranschaulichen, wurden für die BOOT und ROISLIEN Intervalle Unsicherheitsbänder gebootstrapped (b = 400 Wiederholungen). Die Unsicherheit in diesen Verfahren entsteht durch das wiederholte Ziehen mit Zurücklegen im Rahmen des Bootstrapverfahrens (BOOT) bzw. die jeweils unterschiedlichen gezogenen Einzelkurven (ROISLIEN).

Im Falle der POINT Intervalle werden alle Kurven eines Datensatzes verwendet, weshalb keine Schwankung der Intervallgrenzen auftritt. Verschiedene Autoren schlagen jedoch Konfidenzintervalle als Erweiterung ihres ursprünglichen Ansatzes vor, um den sampling error insbesondere kleinere Stichproben (Bland & Altman, 1986; Hamilton & Stamey, 2007; Giavarina, 2015) zu berücksichtigen. Bland & Altman (1986) schätzen den Standardfehler der Differenzen auf ungefähr $\sqrt{3*s² / c}$.

mit $s$ ... Standardabweichung
und $c$ ... Anzahl der Kurven im Datensatz
...
Die vollständige von Bland & Altman vorgeschlagene Formel (95% CI für 95% LoA) lautet damit: 

$(\overline{d} \pm z_{0.975}*s) \pm t_{0.975, n-1}*s*\sqrt{3*s² / c}$, mit $z_{0.975} = 1.96$
